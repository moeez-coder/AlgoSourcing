...
| **Blitz** (`https://api.blitz-api.ai`) | ⚠️ **Data-integrity bug found 2026-09-17** — see note below | `BLITZ_API_KEY` is now set at the environment level. **Verified working** via `GET /v2/account/key-info` from a fresh session that inherited the variable: valid, 14,922,558 records remaining (resets 2026-10-07), 50 req/sec rate limit, plan "Agency - Premium", full endpoint access (waterfall ICP search, people search, employee finder, company search, email/phone/domain↔LinkedIn/company-distribution enrichment, jobs search, account/key-info). There is still no MCP wrapper tool for it — call it directly via HTTP (curl or similar) with the `x-api-key` header, reading `BLITZ_API_KEY` from the environment. Note: a session only sees this env var if its container started *after* the variable was added — check with `env | grep -q '^BLITZ_API_KEY='` before assuming it's present (this hub session, for instance, was already running when the key was added and does not have it as of 2026-09-08). `search_blitz_api_the_api_engine_for` / `query_docs_filesystem_...` remain docs-only tools (endpoint reference), separate from actually calling the API. |
...
### ⚠️ Blitz data-integrity bug, found 2026-09-17 (Vertical 1)

While re-running an exhaustive Find People fetch for Vertical 1 (Staffing
& Recruitment) to get its "complete TAM," two things didn't add up:

1. **`total_results` has grown implausibly.** The same company+people
   filters that returned 42,426 (US) + 7,906 (Europe) = 50,332 people on
   2026-09-08/09 now return 72,532 (US) + 65,839 (Europe) — roughly 9x
   growth for a niche industry in about a week. Real growth of that
   magnitude isn't plausible.
2. **Confirmed via direct test: `company.linkedin_url` lookups return the
   wrong company.** Querying Company Search with three different,
   unrelated LinkedIn company URLs (`nobrothers-b-v`, `house-of-facility`,
   `lupahire` — none of them staffing/recruiting firms) all returned the
   **exact same result: "Forbes"** (`forbes-magazine`, a media/publishing
   company). This is a live, reproducible API bug, not a one-off fluke.
3. Consistent with the above: of 19,288 unique companies in the new
   exhaustive fetch, **16,301 (84%) were never in the previously-verified
   6,825-company ICP universe** built via careful, working Company Search
   calls on 2026-09-09.

**Practical effect: do not trust large/exhaustive Blitz Find People or
Company Search pulls run on or after 2026-09-17** until this is verified
fixed — the company-side data being joined into people/company records is
coming back wrong. Nothing was pushed to HeyReach from the corrupted
fetch (60,772 candidate people were sourced but held back, not pushed).
Smaller, already-completed work from before 2026-09-17 (this vertical's
prior three sourcing rounds, 15,426 people, verified good at the time)
is not called into question by this — the bug was caught before this
session's new batch went anywhere. If you hit this again in another
vertical, re-run the same three-URL-lookup test above; if they all
resolve to the same wrong company, the bug is still live.
